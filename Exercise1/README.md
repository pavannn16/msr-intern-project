...
| **Rounding Mode** | even | Matches the current evaluation configuration |
...
├── results/                        # Notebook-generated outputs (runtime)
│   └── exercise1_results.txt      # (optional) local copy generated by notebook
├── README.md                       # This file
...
mx_specs['round'] = 'even'              # Rounding mode
...
|--------|-----------------|-----------------|--------|
| **Accuracy (MX full)** | 62.10% | 50.67% | -11.43pp |
| **Accuracy (custom_cuda=0)** | 62.10% | 50.84% | -11.26pp |
| **Accuracy (weights-only, a=None)** | 62.10% | 51.76% | -10.34pp |
| **Memory (Weights)** | 4.88 GB | ~1.22 GB | -75% |
| **Memory (Acts)** | Full precision | ~6-bit | -81% |
| **Runtime** | ~22s | Not measured in repo (varies by GPU/runtime) | N/A |

Canonical metrics are recorded in `results/exercise1_results.txt`.
...
**Exercise 1 Status**: ✅ Implementation Complete | ✅ Evaluation Complete (accuracy below target)  
